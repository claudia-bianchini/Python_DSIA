...
def main():
    ...
    # Inserisci qui il codice JavaScript per regolare le dimensioni dei grafici
    app.scripts.append_script({
        'external_url': 'https://code.jquery.com/jquery-3.5.1.slim.min.js'  # Aggiungi jQuery
    })

    app.clientside_callback(
        """
        function resizeCharts() {
            var figHeight = window.innerHeight * 0.5;  // Modifica il valore 0.5 per regolare l'altezza in base alla percentuale desiderata
            var figWidth = window.innerWidth * 0.8;   // Modifica il valore 0.8 per regolare la larghezza in base alla percentuale desiderata

            return [figHeight, figWidth];
        }

        setInterval(function() {
            var dimensions = resizeCharts();
            var figHeight = dimensions[0];
            var figWidth = dimensions[1];

            document.getElementById('north-hist').style.height = figHeight + 'px';
            document.getElementById('north-hist').style.width = figWidth + 'px';

            document.getElementById('scatter-plot').style.height = figHeight + 'px';
            document.getElementById('scatter-plot').style.width = figWidth + 'px';
        }, 500);
        """,
        Output('north-hist', 'style'),
